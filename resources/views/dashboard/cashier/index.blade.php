...
@includeIf('dashboard.cashier.stock')
@includeIf('dashboard.cashier.member')
@endsection
...
<script>
    ...
    function tambahProduk() {
        $.post('{{ route('cashier.store') }}', $('.form-produk').serialize())
            .done(response => {
                $('#kode_produk').focus();
                table.ajax.reload(() => loadForm($('#diskon').val()));
            })
    ...
    function loadForm(diskon = 0, diterima = 0) {
        ...
        $.get(`{{ url('/cashier/loadform') }}/${diskon}/${$('.total').text()}/${diterima}`)
            .done(response => {
